module.exports = function (grunt) {
  var compileType = function () {
      ...
      var files = grunt.file.expand(["src/" + type + "/*.js"]);
  ...
  grunt.initConfig({
    mochaTest: {
      all: ["tests/numeral.js", "tests/formats/*.js", "tests/locales/*.js"],
    },
    karma: {
      options: {
        files: [
  ...
  // TODO: "jshint",
  grunt.registerTask("build", ["compile", "compile:numeral", "copy"]);

  grunt.registerTask("test", ["build", "mochaTest", "karma:local"]);

  grunt.registerTask("test:npm", ["build", "mochaTest"]);

  grunt.registerTask("test:browser", ["build", "karma:local"]);
